...
import { generateEvent, generateEvents, generateUser } from '../src/utils/helper';
...
const createUserWithEventCreationPermission = async () => {
	...
	user.user.permissions = [permission._id];

	const databaseUser = await Member.findById(user.user._id).exec();
	databaseUser.permissions = user.user.permissions;
	...
	describe('Get all events', () => {
		...
		it('Returns all events (including private)', async () => {
			...
			const {
				...
				status
			} = await request.get('/api/events').set('Authorization', `Bearer ${user.token}`);
	...
	describe('Update an event', () => {
		it('Fails to update an event because invalid id', async () => {
			const {
				body: { error },
				status
			} = await request.post(`/api/events/invalidId`);
		...
		it("Fails to update an event because the event doesn't exist", async () => {
			...
			const {
				body: { error },
				status
			} = await request.post(`/api/events/${new ObjectId()}`).send(event);
		...
		it('Fails to update an event because no name', async () => {
			...
			const {
				body: { error },
				status
			} = await request.post(`/api/events/${createdEvent._id}`).send(event);
	...
	describe('Delete an event', () => {
		it('Fails to delete an event because invalid id', async () => {
			const {
				body: { error },
				status
			} = await request.delete(`/api/events/invalidId`);
		...
		it("Fails to delete an event because the event doesn't exist", async () => {
			const {
				body: { error },
				status
			} = await request.delete(`/api/events/${new ObjectId()}`);
		...
		it('Succesfully deletes an event', async () => {
			...
			const {
				body: { response },
				status
			} = await request.delete(`/api/events/${event._id}`);
	...
	describe('Checkin user to event', () => {
		it('Fails to check in user to event because invalid event id', async () => {
			const {
				body: { error },
				status
			} = await request.post('/api/events/invalidId/checkin');
		...
		it("Fails to check in user to event because the event doesn't exist", async () => {
			const {
				body: { error },
				status
			} = await request.post(`/api/events/${new ObjectId()}/checkin`);
		...
		it('Fails to check in user to event because the user has no name', async () => {
			...
			const {
				body: { error },
				status
			} = await request.post(`/api/events/${event._id}/checkin`).send({
				email: 'fake'
			});
	...
	describe('Checkout user from event', () => {
		it('Fails to check out user from event because invalid event id', async () => {
			...
			const {
				body: { error },
				status
			} = await request.delete(`/api/events/invalidId/checkin/${member._id}`);
		...
		it('Fails to check out user from event because event does not exist', async () => {
			...
			const {
				body: { error },
				status
			} = await request.delete(`/api/events/${new ObjectId()}/checkin/${member._id}`);
		...
		it('Fails to check out user from event because invalid member id', async () => {
			...
			const {
				body: { error },
				status
			} = await request.delete(`/api/events/${event._id}/checkin/invalidId`);
		...
		it('Fails to check out user from event because member does not exist', async () => {
			...
			const {
				body: { error },
				status
			} = await request.delete(`/api/events/${event._id}/checkin/${new ObjectId()}`);
		...
		it('Fails to check out user from event because they were never checked in', async () => {
			...
			const {
				body: { error },
				status
			} = await request.delete(`/api/events/${event._id}/checkin/${member._id}`);
